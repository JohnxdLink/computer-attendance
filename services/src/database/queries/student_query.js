...
const FIND_CASE_RECORDS_BY_STUDENT_ID = `SELECT cr.id AS case_record_id, cr.student_id, cr.concern_id, cr.opened_by, cr.opened_date, cr.title, cr.description, cr.priority, cr.status, cr.closed_date, cr.closing_remarks, cr.created_at, cr.updated_at FROM case_records AS cr WHERE cr.student_id = ? ORDER BY cr.id DESC`;
...
const FIND_GUIDANCE_SESSIONS_BY_CASE_RECORD_ID = `SELECT gs.id AS guidance_session_id, gs.case_record_id, gs.staff_id, gs.session_date, gs.session_time, gs.session_type, gs.concern, gs.discussion_summary, gs.intervention, gs.recommendation, gs.follow_up_date, gs.status, gs.created_at, gs.updated_at FROM guidance_sessions AS gs WHERE gs.case_record_id = ? ORDER BY gs.id DESC`;
